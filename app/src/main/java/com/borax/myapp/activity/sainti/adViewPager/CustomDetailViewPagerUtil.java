...
/**
 * Created by borax on 2016/8/10.
 */
public class CustomDetailViewPagerUtil {

    private Context context;
    private ViewPager viewPager;
    private CustomDetailViewPagerAdapter mimageViewPagerAdapter;
     ...
     * @param urls
     */
    public CustomDetailViewPagerUtil(Context context, ViewPager viewPager, List<String> urls) {
     ...
     * @param urls
     */
    public CustomDetailViewPagerUtil(Context context, ViewPager viewPager, LinearLayout dotlayout, int dotsize, int dotoffset, List<String> urls) {
        this.context = context;
        this.viewPager = viewPager;
    ...
    private void setImg(int length, List<String> urls) {
        if (urls.size() > 0) {
            ...
            Glide.with(context).load(urls.get(urls.size() - 1)).into(mImageViews[0]);
            Glide.with(context).load(urls.get(0)).into(mImageViews[length - 1]);
        }
    }
    ...
    public void initVps() {
        initAdimgs(urls);
        initDots(urls.size());
        mimageViewPagerAdapter = new CustomDetailViewPagerAdapter(context, mImageViews);
